...
from matrice_wind_sense.msg import OpticFlowMsg
...
def draw_optic_flow_field(gray_image, points, flow):
    ...
    for i, point in enumerate(points):
        ...
        vx = flow[i][0, 0]
        vy = flow[i][0, 1]
        cv2.line(color_img, (int(x), int(y)), (int(x + vx), int(y + vy)), color_red,
                 linewidth)  # draw a red line from the point with vector = [vx, vy]
...
def define_points_at_which_to_track_optic_flow(image, spacing):
    ...
    points_to_track = np.array(points_to_track,
                               dtype=np.float32)  # note: float32 required for opencv optic flow calculations
    points_to_track = points_to_track.reshape(points_to_track.shape[0], 1, points_to_track.shape[
        1])  # for some reason this needs to be shape (npoints, 1, 2)
...
class Optic_Flow_Calculator:
    def __init__(self, topic):
        ...
        self.image_sub = rospy.Subscriber(self.image_source, CompressedImage, self.image_callback)
    ...
    def image_callback(self, image):
        try:  # if there is an image
            ...
            curr_image = cv2.imdecode(np_arr, 0)
            ...
            new_position_of_tracked_points, status, error = cv2.calcOpticalFlowPyrLK(self.prev_image, curr_image,
...
if __name__ == '__main__':
    ...
    optic_flow_calculator = Optic_Flow_Calculator('/raspicam_node/image/compressed')
